  ...
  try {
    ...
    if (tagsParam) {
      ...
      const surveys = await fetchAllRanges<{
        ...
        let q = supabase
          .from("surveys")
          .select("id, platform, instructor, course, cohort", withCount ? { count: "exact" } : undefined);
        if (platform) q = q.eq("platform", platform);
        return q.range(from, to);
      });
      if (surveys.length === 0) return NextResponse.json([]);
    ...
    if (tag) {
      ...
      // tag가 직접 설정된 댓글 + auto_field 댓글을 병렬 페이지네이션으로 조회
